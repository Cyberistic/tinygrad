...
class TestGLSLESOnDevice(unittest.TestCase):
  """End-to-end test for the GLSL_ES FFI path. Run on the Android device
  via the helper script in android-physical/flower-tinygrad-native/run_e2e_test.sh
  which builds and runs a small C program that exercises the full FFI
  (android_gles_init, buffer_create/upload/download/free, dispatch_jit_kernel,
  last_kernel_time_ns, pop_error) against a tinygrad-generated kernel.

  This test class documents the expected behaviour on-device; it cannot run
  in this Python interpreter because libflower_tinygrad.so is ARM64 Android.
  See app/src/test/native/test_glsl_es_e2e.c for the actual C test.
  """
  @unittest.skip("Runs natively on Android, see app/src/test/native/test_glsl_es_e2e.c")
  def test_on_device_e2e(self):
    # Reference behaviour (asserted by the C test on device):
    #   - kernel_017_fwd.comp (ReLU + 2x2 maxpool, generated by tinygrad)
    #     dispatched on (B=32, C=16, H=32, W=32) random input.
    #   - Output downloaded and compared against numpy CPU reference.
    #   - max_abs_diff == 0.0, max_rel_diff == 0.0, all 131072 elements match.
